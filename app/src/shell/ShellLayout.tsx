...
import { AppModulesPanelContent } from './AppModulesPanelContent'
...
export function ShellLayout({
  ...
  const [appModulesPanelOpen, setAppModulesPanelOpen] = useState(false)

  const handleRightSidebarItemAction = useCallback((itemKey: string) => {
    if (itemKey === 'app-menu') {
      setAppModulesPanelOpen((open) => !open)
    }
  }, [])

  const closeAppModulesPanel = useCallback(() => {
    setAppModulesPanelOpen(false)
  }, [])
  ...
  return (
    ...
    >
      ...
      >
        ...
        {showRightSidebar && (
          <RightSidebar
            ...
            panelOpen={appModulesPanelOpen}
            activeItemKey={appModulesPanelOpen ? 'app-menu' : null}
            onItemAction={handleRightSidebarItemAction}
          />
        )}
      ...
      {showRightSidebar && (
        <ShellPanel
          side="right"
          open={appModulesPanelOpen}
          title="Modules"
          titleIcon="squares-2x2"
          headerVariant="theme"
          onClose={closeAppModulesPanel}
        >
          <AppModulesPanelContent onNavigateAway={closeAppModulesPanel} />
        </ShellPanel>
      )}
    </div>
  )
