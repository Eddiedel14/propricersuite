...
export interface RightSidebarNavItem {
  ...
  label: string
  href?: string
  /** When set, renders a button and calls `onItemAction` instead of navigation. */
  itemKey?: string
  isCustom?: boolean
  customSrc?: string
...
const PPM_SECTIONS: RightSidebarSection[] = [
  ...
  {
    items: [
      { icon: 'squares-2x2', label: 'App menu', itemKey: 'app-menu' },
      { icon: 'bell', label: 'Notifications' },
      { icon: 'question-mark-circle', label: 'Help' },
...
export interface RightSidebarProps {
  ...
  className?: string
  /** Mirrors open shell panel state for Harmony sidebar chrome (`data-panel-open`). */
  panelOpen?: boolean
  /** Highlights the item whose `itemKey` matches (e.g. `app-menu` when modules panel is open). */
  activeItemKey?: string | null
  /** Fired for items that define `itemKey` (e.g. App menu). */
  onItemAction?: (itemKey: string) => void
}
      ...
      {sidebarSections.map((section, sectionIndex) => (
        <div key={sectionIndex} className="right-sidebar__section">
          {section.items.map((item, itemIndex) => {
            ...
            const isActive =
              itemKey != null && activeItemKey != null && itemKey === activeItemKey
